    ...
    const handleClusterUpload = async (file: File) => {
        ...
        try {
            ...
            if (!parsed || Object.keys(parsed).length === 0) {
                throw new Error("No cluster mapping found");
            }
            setClusterMap(parsed);
        } catch (err) {
            setClusterMap(null);
            setClusterError(err instanceof Error ? err.message : "Failed to parse cluster map");
        }
    };
    ...
    return (
        <div className="surface-panel p-6 mb-8">
            <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <p className="section-kicker">Insight Space</p>
                    <h3 className="section-title">4-Quadrant Overview</h3>
                    <p className="text-xs text-muted-foreground mt-1">
                        Evidence sufficiency vs answer integrity (Z is hidden in 2D).
                    </p>
                </div>
            ...
            {granularity === "cluster" && (
                ...
                >
                    <label className="flex items-center gap-2">
                        <span className="font-medium text-foreground">Cluster Map</span>
                        <input
                            type="file"
                            accept=".csv,.json"
                            className="text-xs"
                    ...
                    {clusterMap && (
                        ...
                        >
                            {Object.keys(clusterMap).length} mapped
                        </span>
                    )}
                    {clusterError && (
                        <span
