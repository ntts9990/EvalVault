...
요약
EvalVault는 RAG 시스템의 품질을 일관되게 평가하고 비교할 수 있는 표준 워크플로를 제공하는 것을 목표로 한다. CLI와 FastAPI + React Web UI를 함께 제공해 평가 실행부터 결과 공유까지의 진입 장벽을 낮춘다. 평가 결과는 SQLite/PostgreSQL에 저장되고 Langfuse·Phoenix·MLflow 같은 추적기와 연결된다. 도메인 메모리와 분석 파이프라인을 통해 과거 평가 결과를 재사용하고 개선 방향을 제시하는 데 초점을 둔다. 한국어 RAG 최적화와 임베딩/검색 통합이 핵심 기능 스펙으로 정의돼 있다. 현재 단계는 병렬 리팩토링과 문서 정합성 확보를 통해 제품 품질을 끌어올리는 데 집중한다.
