...
**상태 메모**
- R1 완료 보고서: `docs/internal/reports/R1_COMPLETION_REPORT.md`
...
- 최신 `docs/status/ROADMAP.md`, `docs/internal/reference/DEVELOPMENT_GUIDE.md`, `docs/internal/plans/PARALLEL_WORK_PLAN.md` 확인
...
1) 사전 체크
   ...
   - 상태/샘플 경로는 `docs/internal/status/STATUS.md`만 최신화
...
> **상세 계획**: [CLI_DEVELOPMENT_PLAN.md](./CLI_DEVELOPMENT_PLAN.md)
...
- 일관된 옵션 체계
- 명확한 에러 메시지
- run.py 분리 (1,470 LOC → 500 LOC 이하)

### 진행 현황

| # | 태스크 | 상태 | 비고 |
|---|--------|------|------|
| 1 | QW1: 에러 메시지 개선 | ✅ 완료 | `utils/errors.py` |
| 2 | QW3: 명령어 별칭 추가 | ✅ 완료 | PR #91 |
| 3 | QW4: 설정 검증 | ✅ 완료 | `utils/validators.py` |
| 4 | 프리셋 시스템 | ✅ 완료 | `utils/presets.py` |
| 5 | `evalvault init` | ✅ 완료 | `commands/init.py` |
| 6 | QW2: Progress Bar | 🚧 미착수 | Rich Progress 통합 |
| 7 | 도움말 메시지 개선 | 🚧 미착수 | Examples/See also 추가 |
| 8 | run.py 분리 | 🚧 미착수 | `run_modes/`, `run_output/` |

### 다음 작업 (즉시 착수)

| 작업 | 우선순위 | 예상 기간 |
|------|----------|----------|
| QW2: Progress Bar | 🔴 높음 | 2일 |
| run.py 분리 | 🔴 높음 | 5일 |
| 도움말 메시지 개선 | 🟡 중간 | 3일 |

### 향후 CLI 확장 (Q2-Q3 2026)

| 명령어 | 설명 | 의존성 |
|--------|------|--------|
| `evalvault metric` | 커스텀 메트릭 관리 | Phase 15 |
| `evalvault suggest` | 개선 제안 생성 | Phase 17 |
| `pipeline run` 확장 | YAML 파이프라인 실행 | Phase 18 |
| `kg analyze/visualize` | KG 분석 및 시각화 | Phase 19 |
...
| 4 | 문서 스타일 가이드 | `docs/internal/reference/STYLE_GUIDE.md` (신규) |
