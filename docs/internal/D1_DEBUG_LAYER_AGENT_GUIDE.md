...
- `docs/internal/STATUS.md` (최신 상태/샘플 단일 기준)
...
- JSONL ↔ StageEvent 매핑은 `docs/internal/R3_PROGRESS_REPORT.md` 참고
...
> 샘플 확보 현황과 경로는 `docs/internal/STATUS.md`에서 최신으로 관리합니다.
...
> 예시/확보 현황은 `docs/internal/STATUS.md`에서 최신으로 관리합니다.
...
| 에이전트 | 필수 산출물 | 예시/위치 |
|---------|-------------|-----------|
| R1 | run_id + stage report | `docs/internal/R1_COMPLETION_REPORT.md` |
| R2 | run_id + KG fixture | `tests/fixtures/` |
| R3 | run_id + JSONL 로그 | `scripts/perf/` |
| R4 | ground_truth fixture | `tests/fixtures/` |
| D1 | DebugReport 샘플(초기) | `docs/internal/DEBUG_TOOL_PLAN.md` |
...
- 진행 순서/상태 업데이트는 `docs/internal/STATUS.md`에만 반영
- 계획/설계 변경은 `docs/internal/DEBUG_TOOL_PLAN.md`로 반영
