...
> **구현 완료**: 2026-01-10 | PR #112

| 메트릭 | 정의 | 상태 |
|--------|------|------|
| **HitRate** | 상위 K개 결과 중 관련 문서 포함 여부 | ✅ 완료 |
| **MRR** | 첫 번째 관련 문서의 역순위 | ✅ 완료 |
| **NDCG** | 관련성 등급을 고려한 순위 품질 | ✅ 완료 |

**사용법**:
```bash
# 단일 메트릭
evalvault run data.json --metrics mrr

# 다중 메트릭
evalvault run data.json --metrics mrr,ndcg,hit_rate
```

**특징**:
- 한국어 particle/ending 처리 지원
- 설정 가능한 relevance threshold (기본: 0.3)
- 설정 가능한 K 파라미터 (MRR@K, NDCG@K, HitRate@K)
- Ground truth 기반 관련성 계산

**임계값**: Hit Rate >0.8, MRR >0.6, NDCG >0.7
