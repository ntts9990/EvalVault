...
```bash
uv run evalvault --help
uv run evalvault run data.csv --metrics faithfulness --profile dev
...
uv run evalvault run tests/fixtures/e2e/insurance_qa_korean.json \
  ...
  --profile dev
```
- `run-simple` / `run-full` 별칭: `uv run evalvault run --mode simple/full`과 동일하며 초보자가 dataset + profile만으로 실행할 수 있도록 안내용 도움말을 별도 섹션으로 분리했습니다.
...
### 3.3 `gate`
```bash
uv run evalvault gate RUN123 --format json
```
- 품질 게이트 보고서를 JSON 또는 GitHub Actions 주석으로 출력합니다.

### 3.4 `domain`
```bash
uv run evalvault domain init insurance --languages ko,en --description "보험 QA"
uv run evalvault domain list
uv run evalvault domain show insurance
```
...
uv run evalvault benchmark retrieval tests/fixtures/benchmark/retrieval_ground_truth_min.json \
...
| 명령 | 설명 | 예시 |
|------|------|------|
| `stats` | 도메인(또는 전체) 기준으로 Facts/Behaviors/Learnings 수량을 표시 | `uv run evalvault domain memory stats --domain insurance` |
| `search` | FTS5 기반 사실 검색, 검증 점수 필터 지원 | `uv run evalvault domain memory search "보험료" --domain insurance --min-score 0.6` |
| `behaviors` | 성공률 순 행동 패턴 조회 | `uv run evalvault domain memory behaviors --min-success 0.8` |
| `learnings` | Experiential 학습 기록 조회 | `uv run evalvault domain memory learnings --limit 10` |
| `evolve` | consolidate/forget/decay 실행 (확인 프롬프트 + `--dry-run`) | `uv run evalvault domain memory evolve --domain insurance --yes` |

샘플 출력:

```bash
uv run evalvault domain memory search "청약 철회" --domain insurance
```
...
Usage:
    uv run evalvault <command> [ARGS] [OPTIONS]
