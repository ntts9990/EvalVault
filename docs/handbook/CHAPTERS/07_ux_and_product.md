...
- 현재 제품 개발 포커스(2026-05-29)는 **CLI/API 우선**이다. Web UI 신규 개발은 보류하고, 신규 기능은 CLI와 FastAPI 계약을 먼저 안정화한다.
...
- 신규 기능의 1차 UX는 CLI와 API다.
- Web UI는 기존 화면 유지와 회귀 방지 범위로 제한한다.
- UI가 필요해 보이는 기능도 먼저 CLI 명령, API 응답 스키마, 에러 계약, 테스트를 완성한다.
- Web UI 작업은 명시 요청이 있거나 CLI/API 계약이 충분히 안정된 뒤 별도 라운드로 진행한다.
...
현재 우선순위에서는 아래 역할 분담을 유지하되, 개발 순서는 **CLI/API → Web UI**로 고정한다.
...
- 신규 기능은 Web UI 화면부터 만들지 않는다.
- Web UI는 CLI/API로 이미 검증된 기능을 탐색/공유용으로 노출하는 후속 표면이다.
- 기존 UI가 깨지는 회귀는 수정하지만, 신규 화면/대규모 UX 개편은 현재 기본 작업 범위가 아니다.
...
2026-05-29 기준 Web UI 신규 개발은 보류 상태다. 이 절은 기존 구현과 향후 재개 시 지켜야 할 UX 계약을 기록한다.
...
### 3.3 로컬 개발 UX: API 우선, 프론트 연결은 선택

현재 기본 로컬 개발 확인은 FastAPI API까지다. 프론트/백엔드 연결은 Web UI 작업이 명시된 경우에만 확인한다.
