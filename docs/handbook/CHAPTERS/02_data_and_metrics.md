...
## 32. 멀티턴(Conversation) 데이터셋 메타데이터 규약

EvalVault는 멀티턴(대화형) RAG 평가를 두 가지 경로로 지원한다.

1) `--mode multiturn` 전용 모드(별도 로더/메트릭) — 워크플로 측면은 ch03 §38 참조.
2) **단일 턴 데이터셋을 평탄화(flatten)한 채 메타데이터로 세션/턴을 연결하는 방식** — 기존 로더/평가/분석 파이프라인을 그대로 쓰면서 멀티턴 집계를 가능하게 한다.

본 절은 (2) 경로의 데이터 측면 규약을 정의한다.

### 32.1 필수 메타데이터 키

`test_cases[].metadata`에 아래 키를 넣는다.
...
- `conversation_id` (필수): 같은 대화 세션을 묶는 식별자
- `turn_index` (필수): 턴 순서(정수)
- `turn_id` (선택): 턴 고유 ID (문자열)

### 32.2 누락 시 동작

- `turn_index`가 없으면 대화 순서를 복원할 수 없다.
- `conversation_id`가 없는 케이스는 대화 단위 집계에서 제외된다.

### 32.3 산출물

분석 파이프라인의 `multiturn_analyzer` 모듈이 다음을 만든다.
...
이 결과는 단일 턴 분석과 같은 위치(`reports/analysis/artifacts/analysis_<RUN_ID>/index.json`)에 등록된다.
