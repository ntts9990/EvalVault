...
#### 38.4 단일 턴 데이터셋에서의 멀티턴 집계(metadata 경로)

`--mode multiturn`을 쓰지 않고도, 평탄화된(test_cases가 턴 단위로 흩어진) 단일 턴 데이터셋에 메타데이터를 붙여 분석 단계에서 멀티턴 집계를 만들 수 있다.

데이터 측면 규약은 ch02 §32에 정의되어 있으며(`conversation_id` / `turn_index` / `turn_id`), 워크플로는 다음과 같다.

1. `evalvault run`으로 평가 실행 후 `--auto-analyze` 또는 후속 분석 파이프라인 실행
2. 분석 파이프라인의 `multiturn_analyzer` 모듈이 대화/턴 단위 집계를 산출
...
- `turn_index`가 누락되면 대화 순서를 정확히 복원할 수 없다.
- `conversation_id` 없는 케이스는 대화 단위 집계에서 제외된다.

이 경로는 기존 로더/평가/분석 파이프라인을 변경하지 않고 추가 메타데이터만으로 멀티턴 관점을 얻고 싶을 때 쓴다. `--mode multiturn` 전용 메트릭(turn_faithfulness/context_coherence/drift_rate)이 필요한 시나리오와는 별개 경로다.
