...
> **이번 조정(adjusted):** §0 요약을 v3.1 전략·14개 레포별 본문과 정합화하는 8개 미세 조정. 본문(§A/§B/§C)의 레포별 계획은 *변경 없음*(각 에이전트 소유). §0.7에 조정 사항 매트릭스, §0.8에 외부 SOTA(AlphaProof Nexus) PROPOSAL 5건 + 보강 3건(deeper read P/Q/R), §0.9에 외부 SOTA(Epicure) PROPOSAL 5건, §0.10에 외부 SOTA(AutoScientists) PROPOSAL 5건, §0.11에 외부 SOTA(Robin / FutureHouse) PROPOSAL 5건 추가.
---
## 0. 최종요약 (신규)
### 0.1 한 줄
...
| P | RL-specialist as focused sub-goal tool (Gemini outer reasoning + AlphaProof RL prover를 sub-goal 전용 도구로 호출 — 일반 LLM call과 구분되는 "특수 specialist 호출" 패턴) | **AHO C4 stats(또는 `deslop_eval`)을 SO `engine/judge.py`/PO eval 진입 전 *focused specialist tool*로 호출** — 결정적·sub-second·재현 가능한 grader가 LLM judge보다 cheap한 1차 screening. PO outer-loop에서 EvalVault 호출 전 AHO `compare_ab`(stdlib-only)로 *quick effect-size sanity*, deslop_eval로 문체 결정적 grader 호출 | SO §3 (`judge.py` 확장) / AHO §2·§3 EPIC-003 / reverra-lab §3 deslop_eval / §0.10 #M Analyst 호환 | PROPOSAL — §0.10 #M과 합산 가능 |
| Q | Multi-turn refinement via structured edit tool (Ralph loop의 search-and-replace — regenerate가 아닌 *delta editor*) | **PO mutation을 regenerate → structured edit으로 전환**. 후보 prompt에 search-and-replace tool 인터페이스를 부여 → 작동하는 후보 본체 보존하면서 부분만 변이. token 비용 ↓, §0.10 K champion 안정성과 자연 합산(champion을 *처음부터 재생성*하지 않고 *부분 편집*만) | PO §3 (`mutation.py` operator 일반화) / §0.8 #A / §0.10 K | PROPOSAL — §0.8 #A·§0.10 K 직접 강화 |
| R | Domain-agnostic agent reuse (논문의 같은 agent가 조합론·최적화·그래프이론·대수기하·양자광학 + 217 ProteinGym assay에 *무수정 적용*) | **SO `harness-registry.yaml` + `catalog/repo-catalog.yaml` manifest 패턴 강화** — PO 1 cycle 실증이 모든 repo에 *S1 context*로만 적용되도록 강제, repo-specific 분기는 manifest로만 들어가게(코드 분기 금지). 14개 plan 본문에 박힌 "각 repo가 자기 PO를 따로 구현" 우려 사전 차단 | SO §2 (harness-registry) / §0.4 처리결정 맵 PO 행 / §0.7 #5 reverra-lab S1 unit 분할(14→manifest로 자동 흡수) | PROPOSAL — manifest 표면 표준화만 |
**보강 P·Q·R (deeper read, 2026-05-28):** A-E 외에 추가 식별된 3건. 모두 §0.8 / §0.10과 자연스럽게 합산(P=§0.10 #M Analyst 융합 / Q=§0.8 #A·§0.10 K mutation 강화 / R=manifest 표준화로 PO 횡적 적용 보장).
**권장 착수 순서:** **E(즉시 ADR-0010 작성, 1시간)** → **D(EPIC-002 MVP 완료 후 additive minor, Q2W5+)** → **R(Phase 0 SO manifest 패턴 확인, 작은 작업)** → **C(Q3 LRV 스크립트층 흡수와 합치기)** → **P·Q(§0.10 K·M과 같은 EPIC 묶음)** → **A·B(Q3 EPIC-004 1 cycle 실증 직후 follow-up EPIC)**. 각 항목은 task-bank regression으로 before/after 측정 후 EPIC화 — proposal §5(자기평가 이득=0 회피, held-out 강제)와 정합.
**비차단성·정합성:**
...
### 0.11 외부 SOTA 반영 — Robin / FutureHouse (PROPOSAL, 2026-05-28)
**Source:** *A multi-agent system for automating scientific discovery* (Nature, FutureHouse 2026; arXiv:2505.13400, GitHub Future-House/robin). **명명된 specialist 3 agent**가 *닫힌 루프*(hypothesis → experiment → analysis → refined hypothesis)로 협업: **Crow**(PaperQA2 기반 literature scout) · **Falcon**(deep literature review + OpenTargets 등 구조화 DB) · **Finch**(experimental data analysis — RNA-seq, flow cytometry). **LiteLLM**으로 LLM provider abstraction. **Semi-autonomous** — 사람이 wet-lab 실험 실행, agent가 literature·hypothesis·analysis. **결과:** 약 *900시간의 인지노동 → 2시간 미만*으로 압축, dry-AMD(노인성황반변성) 신규 후보 *ripasudil* 자율 식별. **§0.10 AutoScientists와의 차이:** AutoScientists는 self-organizing(역할 가변), Robin은 *named roles 고정* — 우리 §4 two-control-plane 명시 분리 설계와 자연 정합.
**적용 가능한 패턴 → 우리 컴포넌트 매핑**
| # | Robin 패턴 | 우리 매핑 | 본문 cross-ref | 상태 |
|---|---|---|---|---|
| S | Named specialist agents with distinct tool stacks (Crow=PaperQA2 literature / Falcon=deep review+OpenTargets / Finch=data analysis) — 역할 *고정*, 도구 스택 *분리* | **PO outer-loop를 named sub-role 3종으로 분해**: literature scout(LRV hybrid search backed) · deep review(LRV + KG + 외부 source) · data analyst(AHO C4 stats + EvalVault). §0.10 #M Analyst와 *상호 보완*(M=Analyst 단일 역할, S=다중 named 역할). | PO §3 (`runner.py` sub-agent 분기) / LRV §3 EPIC / AHO §3 EPIC-003 / §0.10 #M | PROPOSAL — §0.10 #M·#L과 묶음 |
| T | Closed-loop with explicit *hypothesis refinement* phase (hypothesis → experiment → analysis → refined hypothesis, *hypothesis는 1급 객체*) | **PO outer-loop에 `Hypothesis` artifact 추가** — 현재 PO는 candidate prompt만 업데이트, hypothesis는 암묵. C1 EvidenceRef로 wrap한 `Hypothesis`(text + evidence_refs + verifier_target) 명시 후 cycle마다 regression 결과로 *refine*. §0.10 K champion이 candidate 트랙이라면, T는 hypothesis 트랙(상위 추상). | PO §2 (계약 입출력에 Hypothesis 추가) / PO §3 (`optimizer.py`) / §0.10 K champion 분리 | PROPOSAL — §0.10 K와 분리된 별 트랙 |
| U | Semi-autonomous with explicit human-in-the-loop boundary (wet lab = 사람, literature/analysis = agent) | **§18.5 Stage 3 (Production gated write) 모델의 외부 검증** — Robin이 wet lab을 사람에게 맡기듯, 우리 Stage 3 production write는 `parent_approval_id + T3 + operator approval` 필수. 외부 SOTA가 동일 boundary 채택. ADR-0010(§0.8 #E)에 *operator-as-final-authority for Stage 3* 인용 추가 | §18.5 Stage 모델 / §0.8 #E ADR-0010 / SO §5 DoD / PO §6 DoD | DESIGN 검증 (본문 변경 0) |
| V | Provider-agnostic LLM abstraction (**LiteLLM**) — 어떤 LLM provider도 동일 인터페이스 | **C6 llm-rest의 직접 구현 후보로 LiteLLM 평가** — 현재 우리 C6 설계(provider-neutral REST/SSE/structured JSON + offline stub)는 LiteLLM 인터페이스와 거의 동형. C6 evidenceops-contracts 발행 시 LiteLLM을 reference impl로 채택 검토 가능(vendor-neutral 보존). | §0.5 C6 llm-rest seed / LRV §3 (gemini plumbing) / reverra-lab §2 (reader provider 어댑터) | PROPOSAL — Q4 C6 추출 시 평가 |
| W | Compressed time-to-discovery as headline metric (**900h → <2h**, *비용*이 아니라 *인지노동 시간*을 메인 지표로) | **CertificationReport에 `time_savings_estimate_hours` 필드 추가** — §0.8 #D `cost_estimate`(달러) 옆에 인지노동 절감 시간 추정. 각 verifier·gate·certification 단계의 *수동 동등 작업 시간*과 비교. Stage 3 승인 기준에 비용 임계와 함께 *time-saved 임계*도 옵션. | solution-platform §3 산출(`certification-report.json`) / §0.8 #D / §19 Capacity Rule(time budget) | PROPOSAL — §0.8 #D와 동시 minor |
**권장 착수 순서:** **U(§0.8 #E ADR-0010과 묶어 즉시, 1시간)** → **V(C6 추출 시 평가, Q4)** → **W(§0.8 #D additive minor와 함께, EPIC-002 MVP 후)** → **T(PO Hypothesis 1급 객체화, EPIC-004 follow-up)** → **S(PO sub-role 분해, §0.10 #M Analyst와 같은 묶음)**. 각 항목 task-bank regression(hypothesis refinement 정확도·time-saved 정확도·sub-role 분리 효과)으로 measurable 검증.
**비차단성·정합성:**
- 본 5건 모두 **추가 작업**, v3.1 critical path 비차단.
- §19 Integration Capacity Rule 준수: U·V·W는 문서/contract minor(작은 작업), T·S는 PO/AHO EPIC 묶음에 합산. 신규 active integration **0 증가**.
- §15.6 Offline gate, mirror-not-fork 보존. LiteLLM 채택 시 vendor-neutral 인터페이스 보존(특정 provider 록인 금지).
**선택적 적용 — 명시적 채택 제외:**
- **End-to-end autonomy(wet lab까지 자동화)** 시도 **금지** — U와 정확히 반대. 우리는 Stage 3 operator approval이 *불변식*. Robin도 wet lab은 사람.
- **PaperQA2 직접 의존 도입 X** — LRV가 이미 동등한 hybrid search + T0-gated answer 갖춤. Crow/Falcon은 *역할 패턴*만 차용, 도구는 LRV.
**§0.8·§0.9·§0.10과의 결합·비교:**
- **§0.8 (AlphaProof Nexus)** = verifier-driven candidate search
- **§0.9 (Epicure)** = embedding geometry + canonicalisation
- **§0.10 (AutoScientists)** = agent self-organization + champion + 실패 경험 누적
- **§0.11 (Robin)** = *named role specialists* + *explicit hypothesis refinement* + *semi-autonomous human-in-loop boundary*
- 핵심 차이: §0.10 AutoScientists는 self-organizing(역할 가변), §0.11 Robin은 *고정 역할* — 우리 §4 *two-control-plane 명시 분리* 설계와 *Robin이 더 자연 정합*. S/T는 §0.10 #M Analyst와 *상호 보완*(M=Analyst 단일 / S=다중 named / T=Hypothesis 1급화). U는 §0.8 #E ADR-0010 외부 입증 4번째 사례(SOTA 4건이 모두 *권위 분리·사람 최종 승인*을 핵심으로 채택).
**SOTA 4건 메타 관찰 (§0.8/§0.9/§0.10/§0.11 통합):** AlphaProof Nexus·Epicure·AutoScientists·Robin 네 SOTA가 모두 *(a) 검증기를 LLM과 분리해 권위 보존*, *(b) 사람 최종 결정 또는 Lean kernel 같은 deterministic ground truth를 경계로 유지*, *(c) cost/time 메트릭을 1급 산출로 노출*함. 우리 §5(권위 사슬)·§15(게이트)·§18.5(Stage 모델)·§19(Capacity Rule)가 *동일 4 패턴*을 사전 채택했다는 점이 외부 검증. **ADR-0010 "Verifier-as-authority + Human-as-final-approver"** 작성 시 4건 모두 인용 가능.
---
